...
export default function Cart() {
  ...
  // const totalPricePerDay = selectedItems.reduce(
  //   (sum, item) => sum + Math.round(item.price / 7) * item.quantity,
  //   0
  // );
  const totalPricePerMonth = selectedItems.reduce(
    (sum, item) => sum + Math.round(item.PricePerMonth * item.quantity),
    0
  );
      ...
      <div className='cart-summary-sticky'>
        <div className='cart-summary'>
          <div className='summary-row'>
            <span>{`${formatPrice(totalPrice)} THB / week`}</span>
          </div>
          <div className='summary-row'>
            <span>{`${formatPrice(totalPricePerMonth)} THB / month`}</span>
          </div>
          <Link
